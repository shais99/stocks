...
export class StocksService {
  ...
  async getStock(symbol: string, username: string): Promise<Stock> {
  ...
  async search(query: string): Promise<Stock[]> {
    try {
      const response = await axios.get(
        `https://financialmodelingprep.com/api/v3/search?apikey=${process.env.FMP_API_KEY}&query=${query}&limit=20`,
      );
  ...
  async getQuote(id: string, username: string): Promise<Quote> {
    ...
    try {
      const response = await axios.get(
        `https://financialmodelingprep.com/api/v3/quote/${stockById.symbol}?apikey=${process.env.FMP_API_KEY}`,
      );
